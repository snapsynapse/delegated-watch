...
- An assistant guide at `/.well-known/assistant-guide.txt`, conforming to the GuideCheck Human-Verifiable Assistant Guide profile 2.0.0 at Level 3. It is the agent-facing install path: explicit scope and non-goals, a compact verification instruction before any action, and one structured action block per step with an approval gate on every networked and code-executing command. Verified with the GuideCheck reference verifier 2.0.0 on 2026-09-22, zero blocking findings. Level 4 is not claimed; it needs a sidecar manifest and an independent cross-channel anchor, and neither is published.
- A sidecar manifest at `/.well-known/assistant-guide-manifest.txt` pinning the guide's SHA-256 and byte count, named from the guide's `manifest-url`, with the same hash cross-published as a DNS TXT record at `_assistant-guide.delegated.watch`. Those three land together by necessity: a guide declaring `manifest-url` is evaluated against Level 4, so publishing the manifest ahead of its anchor would make a fetching verifier report a missing anchor as blocking and tell every assistant not to proceed. Level 4 is not asserted on the page; the manifest's `immutable-release-url` names a release not yet cut, and the claim belongs to a fetching verifier rather than to this repository.
