...
Consumer chat products generally report quotas, message windows, or credits rather than token totals. `config/known-activity.json` already models a surface whose activity is evidenced but not countable, and nothing populates it. A capture that records activity bounds for those surfaces, and nothing more, would make a real part of a person's delegated work visible without contaminating the token record. The gate is strict: a bound must never be rendered as a count.
Promoted above the schema work because it is the only item that makes a currently invisible share of delegated work visible at all. Everything below improves figures the record already carries; this one adds a class of evidence the record has never had, and `config/known-activity.json` has modelled it from the first release with nothing populating it. The risk is the reason it was ranked low, and the risk is manageable by construction: a bound is not a row, and the dashboard renders the two differently.
